...
namespace BeymenCase.SERVICE_A
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                return new ConfigurationReader(applicationName, redisConnectionString, 5000);
            });

            services.AddHostedService(provider =>
            {
                return (ConfigurationReader)provider.GetRequiredService<IConfigurationReader>();
            });

            var rabbitMqSettings = Configuration.GetSection("RabbitMq");
            services.AddSingleton<IMessageConsumer>(provider =>
                new RabbitMqConsumer(rabbitMqSettings["HostName"], rabbitMqSettings["QueueName"]));
        }
